...
const Days = () => {
        ...
        {weekCalendarList.map((week, weekIdx) => (
          <React.Fragment key={weekIdx}>
            {week.map((dateObj, dayIdx) => (
              <div
                key={dayIdx}
                className="text-center relative border-t pt-2 min-h-[100px]"
              >
  ...
  return (
    <div className="grid grid-cols-7 flex-grow">
      {weekCalendarList.map((week, weekIdx) => (
        <React.Fragment key={weekIdx}>
          {week.map((dateObj, dayIdx) => {
            ...
            const isToday = dateObj.day === today.getDate() && 
      ...
      {isShowModal && (
        <ScheduleModal DateObj={dateObj} schedule={modalSchedule} close={closeModal} />
      )}
    </div>
  );
};
