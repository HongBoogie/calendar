...
상태 주입은 Context API를 적극적으로 사용했습니다.

미들웨어를 통해 Localstorage에 저장 및 참조를 하게 구현하였고, CRUD 기능을 구현하였습니다.

### 모달

모달을 띄우기 위해 **GlobalPortal** 을 구현하였습니다. 부스트캠프 활동을 하면서 배운 팁인데, Portal을 통해 Modal 뼈대를 구현해놓으면 굉장히 재사용성이 높습니다 (비슷한 로직으로 Toast도 구현하면 괜찮았습니다). 그 후 Modal을 활용해 일정 추가 모달, 일정 상세 모달, 일정 수정 모달을 구현했습니다. 디자인은 좀 손봐야 할듯 싶습니다.
...
### 다른 날짜 요일의 opacity가 불들어와 있는 문제

원래 오늘에 해당하는 요일만 opacity를 100으로 주었으나, 프로덕션 모드에선 다른 요일의 opacity가 100이 되어있는 문제가 있었습니다. 왜 이런일이 일어났을까 생각해보니, SSR로 인한 시간대 차이가 아닌가라는 생각이 들었고, 
기존의 커스텀 훅에서 currentDate를 받아서 바로 띄워주는 것을 useEffect 훅을 이용해 클라이언트 사이드에서 연산 로직을 수행함으로써 해결할 수 있었습니다.
